...
docker run -it --rm -d\
    ...
    --net=host \
    --name=component_stella_docker \
    component_stella_docker:latest \
    ros2 run rclcpp_components component_container_mt

for (( c=1; c<=$1; c++ ))
do
  echo "Load process $i"
  docker exec -it -d component_stella_docker /bin/bash -c ". /ros2_ws/install/setup.bash && ros2 component load /ComponentManager component component_stella::Component"
  sleep 0.2
done
...
docker kill component_stella_docker
